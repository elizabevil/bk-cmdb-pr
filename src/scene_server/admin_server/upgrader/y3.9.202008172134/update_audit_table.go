...
func reconcileAuditTableIndexes(ctx context.Context, db dal.RDB, conf *upgrader.Config) error {
	indexes := []types.Index{
		{Name: "index_id", Keys: bson.D{{Key: common.BKFieldID, Value: 1}}, Background: true},
		{Name: "index_operationTime", Keys: bson.D{{Key: common.BKOperationTimeField, Value: 1}}, Background: true},
		{Name: "index_user", Keys: bson.D{{Key: common.BKUser, Value: 1}}, Background: true},
		{Name: "index_resourceName", Keys: bson.D{{Key: common.BKResourceNameField, Value: 1}}, Background: true},
		{Name: "index_operationTime_auditType_resourceType_action", Keys: bson.D{
			{Key: common.BKOperationTimeField, Value: 1},
			{Key: common.BKAuditTypeField, Value: 1},
			{Key: common.BKResourceTypeField, Value: 1},
			{Key: common.BKOperationDetailField + "." + common.BKObjIDField, Value: 1},
			{Key: common.BKActionField, Value: 1},
		}, Background: true},
	}
