...
// getModuleIDsFromSet
func (ps *ProcServer) getModuleIDsFromSet(ctx *rest.Contexts, bizID int64, input *metadata.ListProcessRelatedInfoOption,
) ([]int64, error) {
	...
	moduleResult, err := ps.CoreAPI.CoreService().Instance().ReadInstance(
		ctx.Kit.Ctx,
		ctx.Kit.Header,
...
func (ps *ProcServer) getServiceInstanceIDs(ctx *rest.Contexts, bizID int64, moduleIDs []int64,
	...
	if err != nil {
		blog.Errorf("getServiceInstanceIDs: GetDistinctField failed, err: %v, option: %#v, rid: %s",
			err, *option, ctx.Kit.Rid)
		ctx.RespAutoError(err)
		return nil, err
	}
...
func (ps *ProcServer) getProcessIDs(ctx *rest.Contexts, bizID int64, serviceInstanceIDs []int64) ([]int64, error) {
...
func buildFinalFilter(
	ctx *rest.Contexts, bizID int64, processIDs []int64, input *metadata.ListProcessRelatedInfoOption,
...
func (ps *ProcServer) fetchProcessDetails(ctx *rest.Contexts,
	filter map[string]interface{}, fields []string, page metadata.BasePage) (*metadata.InstDataInfo, error) {
	...
	result, err := ps.CoreAPI.CoreService().Instance().
		ReadInstance(ctx.Kit.Ctx, ctx.Kit.Header, common.BKInnerObjIDProc, query)
	if err != nil {
