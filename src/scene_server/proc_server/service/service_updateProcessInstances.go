...
func (ps *ProcServer) updateProcessInstances(kit *rest.Kit, input metadata.UpdateRawProcessInstanceInput) ([]int64, errors.CCErrorCoder) {
	bizID := input.BizID

	processIDs, err := ps.decodeAndValidateProcesses(kit, &input)
	if err != nil {
		return nil, err
	}

	relations, err := ps.listProcessRelations(kit, bizID, processIDs)
	if err != nil {
		return nil, err
	}

	if err := ps.validateProcessesExist(kit, processIDs, relations.Info); err != nil {
		return nil, err
	}

	hostIDs := ps.extractHostIDs(relations.Info)
	processTemplateMap, err := ps.getProcessTemplates(kit, relations.Info)
	if err != nil {
		return nil, err
	}
	...
	processDataMap, err := ps.buildProcessUpdateData(kit, input, relations.Info, processTemplateMap, hostMap)
	if err != nil {
		return nil, err
	}

	if err := ps.batchUpdateProcessInstances(kit, processDataMap); err != nil {
		return nil, err
	}
