...
func (ps *ProcServer) createProcessInstances(ctx *rest.Contexts, input *metadata.CreateRawProcessInstanceInput) (
	...
	if err != nil {
		blog.Errorf(
			"create process instance failed, get service instance by id failed, serviceInstanceID: %d, err: %v, rid: %s",
	...
	if serviceInstance.ServiceTemplateID != common.ServiceTemplateIDNotSet {
		blog.Errorf("create process instance failed, "+
			"create process instance on service instance initialized by template forbidden, "+
			"serviceInstanceID: %d, err: %v, rid: %s", input.ServiceInstanceID, err, ctx.Kit.Rid)
