...
func (lgc *Logics) SearchProperty(pHeader http.Header, params *meta.NetCollSearchParams) (*meta.SearchNetProperty,
	...
	if 0 < len(propertyCond) {
		...
		if len(propertyIDs) == 0 || len(objIDs) == 0 {
...
func (lgc *Logics) assembleDeviceShowFieldValue(deviceData []meta.NetcollectDevice,
	...
	if nil == deviceData || len(deviceData) == 0 {
...
func (lgc *Logics) getPropertyIDsAndShowFields(pHeader http.Header, propertyCond map[string]interface{}) ([]string,
	...
	if len(objIDs) == 0 || len(propertyIDs) == 0 || len(propertyIDMapPropertyShowFields) == 0 {
...
func (lgc *Logics) assembleAttrShowFieldValue(attrData []meta.Attribute) (
	...
	if nil == attrData || len(attrData) == 0 {
...
func (lgc *Logics) getDeviceShowField(pHeader http.Header, deviceIDs []uint64) (map[uint64]deviceShowField, error) {
	...
	if len(deviceIDMapDeviceShowFields) == 0 {
...
func (lgc *Logics) getPropertyShowField(pHeader http.Header, propertyIDs []string) (map[string]propertyShowField,
	...
	if len(propertyIDMapPropertyShowFields) == 0 {
