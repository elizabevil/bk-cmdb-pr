...
func (o *ListProcessRelatedInfoOption) Validate() (rawError cErr.RawErrorInfo) {
	if o.ProcessPropertyFilter != nil {
		if key, err := o.ProcessPropertyFilter.Validate(
...
// processTemplateProperty 封装判断函数
func processTemplateProperty[T comparable](templateValue, instanceValue *T,
	process map[string]interface{}, key string) bool {
...
func (pt *ProcessTemplate) ExtractChangeInfo(i *Process, host map[string]interface{}) (mapstr.MapStr, bool, error) {
	...
	if IsAsDefaultValue(t.ProcNum.AsDefaultValue) {
		changed = processTemplateProperty(t.ProcNum.Value, i.ProcNum, process, "proc_num")
	}
	...
	if IsAsDefaultValue(t.StartParamRegex.AsDefaultValue) {
		changed = processTemplateProperty(t.StartParamRegex.Value,
			i.StartParamRegex,
			process,
