...
func (em *EventMetrics) CollectBasic(e *types.Event, tenantID string) {
	// increase event's total count with operate type
	em.totalEventCount.With(prometheus.Labels{"action": string(e.OperationType), metrics.LabelTenantId: tenantID}).Inc()

	// the time when the event is really happens.
	at := time.Unix(int64(e.ClusterTime.Sec), int64(e.ClusterTime.Nano))

	// unix time in seconds
	em.lastEventTime.With(prometheus.Labels{metrics.LabelTenantId: tenantID}).Set(float64(at.Unix()))

	// calculate event lags, in seconds
	lags := time.Since(at).Seconds()

	// set last lag duration
	em.lastEventLagDuration.With(prometheus.Labels{metrics.LabelTenantId: tenantID}).Set(lags)
